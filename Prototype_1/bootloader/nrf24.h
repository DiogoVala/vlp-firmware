...
static int nrf24_init(void) {
	...
	/* Address width */
	nrf24_write_reg(SETUP_AW, (NRF24_ADDR_WIDTH-2) << AW); 
	
	/* Check the register we just configured */
	uint8_t regval = nrf24_read_reg(SETUP_AW);
	if (regval != (NRF24_ADDR_WIDTH-2) << AW)  /* There may be no nRF24 connected */
		return 1;
	
	// Set RF channel
	nrf24_write_reg(RF_CH, NRF24_CHANNEL);

	/* Dynamic payload length for TX & RX (pipes 0 and 1) */
	nrf24_write_reg(DYNPD,(1<<DPL_P0)|(1<<DPL_P1)|(0<<DPL_P2)|(0<<DPL_P3)|(0<<DPL_P4)|(0<<DPL_P5));
	
	/* Enable dynamic payload feature */
	nrf24_write_reg(FEATURE, 1 << EN_DPL);
	
	/* Speed: 2mbps, TX gain: 0dbm */
	nrf24_write_reg(RF_SETUP, (1 << RF_PWR_LOW) | (1 << RF_PWR_HIGH) | (0 << RF_DR_LOW) | (1 << RF_DR_HIGH));

	/* Enable ACKing on pipes 0 & 1 */
	nrf24_write_reg(EN_AA,(1<<ENAA_P0)|(1<<ENAA_P1)|(0<<ENAA_P2)|(0<<ENAA_P3)|(0<<ENAA_P4)|(0<<ENAA_P5));

	/* Auto retransmit delay: 2000 us and Up to 15 retransmissions */
	nrf24_write_reg(SETUP_RETR,(0x07<<ARD)|(0x0F<<ARC));
