...
void setupTimer(led_t *ledp) {
    ...
    switch (ledp->ledMode) {
        ...
        case LED_MODE_DC:
			TIMSK1 = (0 << OCIE1A) | (0 << OCIE1B); /* Disables interrupts on output compare for OCR1A and OCR1B */
            stopTimer();
            break;

        case LED_MODE_ARBITRARY:
            compA = ((F_CPU / getLedFrequency(ledp)) - 1);
        ...
        case LED_MODE_VPPM:
            compA = (uint16_t)((F_CPU / getLedFrequency(ledp)) - 1); /* Define o periodo de um bit */
            OCR1A = compA;
